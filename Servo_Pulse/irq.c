...
uns8 volatile PWM_Pause = 10; 	// Time added in between PWM to slow down the repeat rate of the Output signals 
...
volatile uns16 PID_Delay = 0;	// Count down var for PID timing.   ISR counts down to 0 at ~1ms intervals -- 
...
void high_isr_handler(void)
{
	...
	if( INTCONbits.T0IF )  //  
	{
		...
		if (Ch_Off )			// If there is something to be turned off , i.e. not in Pause, then setup TMR2 
		{
			...
			if ( PR2 > _Maximum)	// limit to max pulse with 
				PR2 = _Maximum;
